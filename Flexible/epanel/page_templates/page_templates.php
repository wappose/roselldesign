...
if ( ! function_exists( 'et_ptemplate_meta' ) ){
	function et_ptemplate_meta($callback_args) {
		...
		$et_ptemplate_projectcats = isset( $temp_array['et_ptemplate_projectcats'] ) ? (array) $temp_array['et_ptemplate_projectcats'] : array();
		...
		<div style="margin: 13px 0 11px 4px; display: none;" class="et_pt_project_categories">
			<h4><?php esc_html_e( 'Select Project categories:', $themename ); ?></h4>
					
		<?php 
			$project_cats_array = get_categories('taxonomy=project_category&hide_empty=0');
			$site_cats = array();
			foreach ($project_cats_array as $categs) {
				$checked = '';
			
				if (!empty($et_ptemplate_projectcats)) {
					if (in_array($categs->cat_ID, $et_ptemplate_projectcats)) $checked = "checked=\"checked\"";
				} ?>
			
			<label style="padding-bottom: 5px; display: block;" for="<?php echo esc_attr( 'et_ptemplate_projectcats-' . $categs->cat_ID ); ?>">
				<input type="checkbox" name="et_ptemplate_projectcats[]" id="<?php echo esc_attr( 'et_ptemplate_projectcats-' . $categs->cat_ID ); ?>" value="<?php echo esc_attr($categs->cat_ID); ?>" <?php echo $checked; ?> />
				<?php echo esc_html( $categs->cat_name ); ?>
			</label>							
		<?php } ?>
		</div>
...
function et_ptemplate_save_details( $post_id, $post ){
	...
	if ( !in_array( $_POST["page_template"], array('page-blog.php', 'page-sitemap.php', 'page-gallery.php', 'page-search.php', 'page-login.php', 'page-contact.php', 'page-template-portfolio.php', 'template-page-projects.php') ) )
		return $post_id;
	...
	if ( 'page-blog.php' == $_POST["page_template"] ) {
		...
		if (isset($_POST["et_ptemplate_blogcats"])) $temp_array['et_ptemplate_blogcats'] = (array) $_POST["et_ptemplate_blogcats"];
	...
	if ( 'template-page-projects.php' == $_POST["page_template"] ) {
		if (isset($_POST["et_ptemplate_projectcats"])) $temp_array['et_ptemplate_projectcats'] = (array) $_POST["et_ptemplate_projectcats"];
	}
