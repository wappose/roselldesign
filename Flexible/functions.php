...
if ( ! function_exists( 'et_postinfo_meta' ) ){
	function et_postinfo_meta( $postinfo, $date_format, $comment_zero, $comment_one, $comment_more, $project = false ){
		...
		if ( in_array( 'author', $postinfo ) ){
			$postinfo_meta .= ' ' . esc_html__('av',$themename) . ' ' . et_get_the_author_posts_link();
		}

		if ( in_array( 'date', $postinfo ) )
			$postinfo_meta .= ' ' . esc_html__(' ',$themename) . ' ' . get_the_time( $date_format );

		if ( in_array( 'categories', $postinfo ) )
			$postinfo_meta .= ' ' . esc_html__(' ',$themename) . ' ' . ( $project ? get_the_term_list( get_the_ID(), 'project_category', '', ', ', '' ) : get_the_category_list(', ') );

		if ( in_array( 'comments', $postinfo ) )
			$postinfo_meta .= ' | ' . et_get_comments_popup_link( $comment_zero, $comment_one, $comment_more );

		if ( '' != $postinfo_meta ) $postinfo_meta = __(' ',$themename) . ' ' . $postinfo_meta;
...
function et_show_ajax_project(){
	...
	while ( $portfolio_query->have_posts() ) : $portfolio_query->the_post();
			...
			if ( $media ){
				...
				foreach( (array) $media as $et_media ){
					...
					if ( is_numeric( $et_media ) ) {
						...
						$video_embed = apply_filters( 'the_content', $wp_embed->shortcode( '', $et_media ) );
		...
		echo strlen($intro) > 0 ? $intro : truncate_post( 560, false );
